...
class WxPayDataBase
{
	...
	* 设置签名，详见签名生成算法
	* @param string $value 
	* @param thirdinfo 必要的平台账户信息
	**/
	public function SetSign($thirdappinfo)
	{
		// $sign = $this->MakeSign();
		$sign = $this->MakeSignByThird($thirdappinfo['wxkey']);
	...
	**/
	 ...
	 * 平台生成签名
	 */
	public function MakeSignByThird($key)
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
		$string = $this->ToUrlParams();
...
class WxPayResults extends WxPayDataBase
{
	 ...
	 * 检测签名
	 */
	public function CheckSign($thirdappinfo)
	{
		//fix异常
		if(!$this->IsSignSet()){
     ...
     * @throws WxPayException
     */
	public static function Init($xml,$thirdappinfo)
	{	
		$obj = new self();
		$obj->FromXml($xml);
		//fix bug 2015-06-29
