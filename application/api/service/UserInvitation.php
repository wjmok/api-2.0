...
class UserInvitation{
    ...
    public function checkinfo($token,$id) 
    {
        ...
        if ($invitationinfo) {
            ...
            if ($userinfo['id']!=$invitationinfo['user_id']) {
                return -4;
            }
        }else{
            return -3;
    ...
    //获取指定类别帖子的回复数
    public function getMenuRemark($id)
    {   
        $map['map']['invitation_id'] = $id;
        $invitationlist = InvitationModel::getInvitationList($map);
        $invitationIDs = array();
        foreach ($invitationlist as $item) {
            array_push($invitationIDs, $item['id']);
        }
        $search['map']['invitation_id'] = array('in',$invitationIDs);
        $remarklist = RemarkModel::getRemarkList($search);
        $remarknum = count($remarklist);
        return $remarknum;
    }
}